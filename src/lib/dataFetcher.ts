import type { Subject, FlattenedSubject, GroupedSubject } from '../types/subject';
...
export async function fetchSubjectDescriptions(grade: number): Promise<Record<string, string>> {
    ...
    const descriptionIndex = headerCells.findIndex((header) => header.includes('คำอธิบาย') || header.includes('แนะนำ') || header.includes('คำแนะนำ'));
        ...
        if (hasMultipleGroups) {
            ...
            for (let i = 0; i < groups.length; i++) {
                ...
                flattened.push({
                    ...
                    note: (notes[i] || notes[0] || '').trim().replace(/^-\s*$/, '') || '',
            ...
            flattened.push({
                ...
                electiveQuantity: electiveQty,
                updatedElectiveQuantity: updatedQty,
                note: (subject.note as string || '').trim().replace(/^-\s*$/, '') || '',
                parsedTimeSlots: parseThaiTime(classtime),
                availableSeats: parseInt(updatedQty, 10) || parseInt(electiveQty, 10) || parseInt(enrollment || '0', 10),
            });
        }
